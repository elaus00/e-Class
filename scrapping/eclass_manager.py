...
class EclassManager:
    ...
    def run(self):
        if not self.eclass.login():
            logging.error("로그인 실패")
            return

        while True:
            courses = self.eclass.get_course_list()
            self._display_courses(courses)
            selected_course = self._select_course(courses)
            if selected_course is None:
                break

            self._handle_course_menus(selected_course)
    ...
    def _handle_course_menus(self, course):
        course_menus = self.eclass.get_course_menus(course.id)
        if not course_menus:
        ...
        while True:
            ...
            menu_type, menu_data = self._select_menu(course_menus)
    ...
    def _select_menu(self, course_menus):
        while True:
            try:
