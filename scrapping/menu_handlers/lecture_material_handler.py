...
class LectureMaterialMenuHandler:
    ...
    async def handle(self, menu_data: Dict[str, str] = None) -> None:
        ...
        await self.get_lecture_material_list()

    # 강의 자료 목록을 가져오고 처리하는 메소드
    async def get_lecture_material_list(self, start: int = 1, search_value: str = '') -> Optional[Tuple[List[Dict[str, str]], Dict[str, str]]]:
        ...
        # 강의 자료 목록 가져오기
        lecture_material_list, parsed_params = await self.scraper.fetch_and_parse_list(
            url=list_url,
            params=params,
